...
export async function loadInvoiceForDownload(invoiceId: string, tenantId: string): Promise<RenderInput> {
  ...
  const paid = (inv.payments_received || []).reduce((s: number, p: any) => s + Number(p.amount || 0), 0);
  ...
  const data: InvoiceData = {
    ...
    company_email: "",
    // Pre-labelled so the header line reads exactly "TIN: 123456789" (or the
    // BR number when no TIN), matching the built-in vector PDF.
    company_tax_number: tenant?.tax_id
      ? `TIN: ${tenant.tax_id}`
    ...
    customer_tax_id: customer.vat_number || customer.tin || "",
