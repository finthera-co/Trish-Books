...
export default function CustomersPage() {
  ...
  // customer_code intentionally omitted: it is auto-generated by the
  // set_customer_code trigger on insert and immutable afterwards.
  const buildPayload = () => ({
    name: form.name.trim(),
              ...
              <TabsContent value="general" className="space-y-4 pt-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Input value={form.customer_code} disabled placeholder="Auto-generated (e.g. CUST-0001)" />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
              ...
              <TableBody>
                {(customers || []).map((c: any) => (
                  <TableRow key={c.id} className="cursor-pointer hover:bg-muted/30" onClick={() => navigate(`/sales/customers/${c.id}`)}>
                    <TableCell className="font-mono text-muted-foreground">{c.customer_code || "—"}</TableCell>
