...
export default function Signup() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Provision the tenant + Company Admin user row server-side. The RPC forces
      // the role and pins auth_user_id to the caller, so signup cannot be used to
      // self-assign a privileged role or join another tenant.
      const { error: provisionError } = await supabase.rpc("signup_provision", {
      ...
      <div className="w-full max-w-md animate-fade-in">
        ...
        <form onSubmit={handleSubmit} className="bg-card rounded-xl border border-border p-6 space-y-4 shadow-sm">
          ...
          <div>
            ...
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className={inputClass} placeholder="••••••••" minLength={8} required />
