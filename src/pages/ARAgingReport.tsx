...
import { useARAging, useARReconciliation } from "@/hooks/useARModule";
...
export default function ARAgingReport() {
  ...
  const { data: aging, isLoading: agingLoading, refetch: refetchAging } = useARAging(asOfDate);
  const { data: recon, isLoading: reconLoading, refetch: refetchRecon } = useARReconciliation(asOfDate);
    ...
    <div className="space-y-6 print:space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Outstanding receivables by age from due date</p>
          </div>
        </div>
      ...
      {recon && (
        <div className={`rounded-lg border px-5 py-4 flex flex-wrap items-center gap-6 ${
          ...
          <div className="flex flex-wrap gap-6 text-sm">
            ...
            <div>
              <p className="text-xs text-muted-foreground">GL Control Account</p>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-6 lg:grid-cols-7 gap-3">
        ...
        <Card className="shadow-sm border-foreground/20">
          <CardContent className="pt-3 pb-3">
            <p className="text-xs text-muted-foreground font-semibold">Total AR</p>
      ...
      <Card>
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <p className="text-center py-10 text-muted-foreground">No outstanding receivables</p>
          ) : (
      ...
      <p className="text-xs text-muted-foreground text-center print:block">
        Report as of {asOfDate} — aging buckets are from invoice due date. Click a row to view the customer.
      </p>
    </div>
  );
}
