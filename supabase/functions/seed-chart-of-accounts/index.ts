...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (existing && existing.length > 0) {
      return new Response(
        JSON.stringify({ success: true, message: "Already seeded", skipped: true }),
    ...
    const { data: cats, error: catErr } = await adminClient
      ...
      .insert(DEFAULT_CATEGORIES.map(c => ({ ...c, tenant_id })))
      .select();
    ...
    const { error: accErr } = await adminClient.from("accounts").insert(accountRows);
    if (accErr) throw new Error(`Account error: ${accErr.message}`);

    return new Response(
      JSON.stringify({ success: true, categories: cats?.length || 0, accounts: accountRows.length }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  } catch (error: any) {
    return new Response(
