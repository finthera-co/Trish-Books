-- ═══════════════════════════════════════════════════════════════════════════
-- PETTY CASH IMPORT — rectify a held row and post the remainder
--
-- Excluding a row got a batch posted, and then stranded that row forever: the
-- resolver refused to touch a posted batch and posting refused to run twice.
-- A grand-total row is right to exclude, but a row held back for a typo — the
-- "31/05/204" date — is a real transaction that must still reach the ledger.
--
-- Three changes make that possible without ever double-posting:
--   * the resolver runs on a posted batch (it only ever touches lines that are
--     not yet posted)
--   * posting accepts a posted batch and works on whatever is ready now,
--     refusing outright when nothing is
--   * rectify_petty_cash_import_line corrects the DERIVED date and account of
--     a held row while leaving the raw cell text untouched, so the sheet as
--     supplied is still on the record beside the correction
--
-- Amount is deliberately not rectifiable. Phase A re-derives it from the raw
-- cell on every resolve, so an override here would be silently discarded; a
-- wrong amount is a wrong sheet, and the sheet is what gets corrected.
-- ═══════════════════════════════════════════════════════════════════════════

-- Let the resolver run on a posted batch. It resets only lines that are
-- neither posted nor excluded, so posted work is untouchable by construction.
...
BEGIN
  ...
  IF v_status NOT IN ('draft', 'resolved', 'failed', 'posted') THEN
    RAISE EXCEPTION 'BATCH_NOT_RESOLVABLE: batch is % — a reverted batch cannot be re-resolved', v_status;
  END IF;
  ...
  SET status = CASE WHEN status = 'posted' THEN 'posted' ELSE 'resolved' END,
      resolved_at = now()
...
BEGIN
  ...
  END IF;
  -- 'posted' is allowed so a batch can post its REMAINDER: rows excluded or
  -- blocked at the first attempt, since corrected. Lines already posted are
  -- never revisited, so this can never double-post.
  IF v_status NOT IN ('resolved', 'posted') THEN
    RAISE EXCEPTION 'BATCH_NOT_RESOLVED: batch is % — resolve it before posting', v_status;
  END IF;
  ...
  FROM petty_cash_import_lines
  WHERE batch_id = p_batch_id AND tenant_id = v_tenant AND status IN ('ok', 'suspense');

  IF v_ready = 0 THEN
    RAISE EXCEPTION
      'NOTHING_TO_POST: every line in this batch is already posted or excluded. Restore and correct a row first.';
  END IF;
  ...
  -- Scoped to the lines THIS run posted. Counting every posted line would
  -- restate the first run's totals on top of the second and make the closing
  -- balance wrong.
  SELECT count(*),
...
-- Correct a held row's derived date and/or account. Raw cell text is never
-- touched, so the results grid keeps showing what the sheet actually said.
CREATE OR REPLACE FUNCTION public.rectify_petty_cash_import_line(
  p_line_id     UUID,
...
DECLARE
  v_tenant UUID := get_user_tenant_id();
  v_line   petty_cash_import_lines%ROWTYPE;
BEGIN
  SELECT * INTO v_line FROM petty_cash_import_lines
  WHERE id = p_line_id AND tenant_id = v_tenant;

  IF v_line.id IS NULL THEN
    RAISE EXCEPTION 'LINE_NOT_FOUND: line % not found for this tenant', p_line_id;
  END IF;

  -- A posted line is in the ledger. Correcting it there is a reversal, not an
  -- edit of the staging row.
  IF v_line.status = 'posted' THEN
    RAISE EXCEPTION
      'LINE_POSTED: this row is already in the ledger. Reverse the import to change it.'
      USING ERRCODE = 'P0007';
  END IF;

  IF p_account_id IS NOT NULL
     AND NOT EXISTS (SELECT 1 FROM accounts WHERE id = p_account_id AND tenant_id = v_tenant) THEN
    RAISE EXCEPTION 'ACCOUNT_NOT_FOUND: account % does not belong to this tenant', p_account_id;
  END IF;

  UPDATE petty_cash_import_lines
  SET parsed_date         = coalesce(p_parsed_date, parsed_date),
      resolved_account_id = coalesce(p_account_id, resolved_account_id),
      resolution_tier     = CASE WHEN p_account_id IS NOT NULL THEN 'manual' ELSE resolution_tier END,
      -- Back to pending so the resolver re-applies every check rather than the
      -- UI declaring the row fixed.
      status              = 'pending',
      error_code          = NULL,
      error_message       = NULL
  WHERE id = p_line_id AND tenant_id = v_tenant;

  RETURN jsonb_build_object('line_id', p_line_id, 'batch_id', v_line.batch_id);
END;
$$;

REVOKE ALL ON FUNCTION public.rectify_petty_cash_import_line(UUID, DATE, UUID) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.rectify_petty_cash_import_line(UUID, DATE, UUID) TO authenticated;

COMMENT ON FUNCTION public.rectify_petty_cash_import_line(UUID, DATE, UUID) IS
  'Corrects the derived date and/or account of a staged import line that is not yet posted, leaving raw cell text intact, and returns it to pending for the resolver to re-check.';
