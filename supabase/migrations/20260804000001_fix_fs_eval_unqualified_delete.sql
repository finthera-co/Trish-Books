-- Fix: "DELETE requires a WHERE clause" when rpc_fs_statement / rpc_fs_coverage
-- are called through PostgREST from the browser.
--
-- fn_fs_eval_statement clears its per-call scratch temp table with a plain
-- `DELETE FROM fs_eval_values;`. Something in this instance's platform-level
-- session guards (supautils / plan_filter, loaded via session_preload_libraries)
-- rejects unqualified DELETE statements outright, even against a freshly
-- created TEMP TABLE with no rows to lose. `supabase db query` runs as a plain
-- superuser session and isn't subject to the same guard, so this was never
-- caught by SQL-level testing — same class of gap as the earlier read-only
-- transaction bug.
--
-- Fix: add a tautological WHERE clause. Functionally identical, satisfies the
-- guard. Body is otherwise unchanged from the current deployed version.
CREATE OR REPLACE FUNCTION public.fn_fs_eval_statement(p_statement_id uuid, p_date_from date, p_date_to date)
