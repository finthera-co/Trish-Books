-- Gap 1 — pro-rate mid-period joiners / leavers.
-- expected_days (the pro-rata denominator) stayed the whole period, so days before
-- hire / after termination were counted as ordinary ABSENCE. Pay came out right by
-- coincidence for biometric monthly staff, but the days were mislabelled and the
-- manual / non-biometric path over-paid joiners. Clip absence to the employment
-- window and surface non_employed_days as a distinct unpaid bucket.

ALTER TABLE public.employees
  ADD COLUMN IF NOT EXISTS termination_date date;

-- Return shape gains non_employed_days, so drop the prior signature first.
