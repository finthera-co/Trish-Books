-- ═══════════════════════════════════════════════════════════════════════════
-- BANK IMPORT — Undo follows the reversal chain.
--
-- journal_entries.reversal_of is a self-FK: a reversal entry points at the
-- entry it reverses. If any bank-import entry was later reversed (via the
-- Journal Entries screen, or a prior batch reverse), deleting the original
-- failed with journal_entries_reversal_of_fkey.
--
-- Undoing the whole import should remove the whole chain: the postings, the
-- suspense-clearing reclass entries, AND anything that reverses them
-- (transitively). Original + reversal net to zero, so deleting both leaves the
-- books correct. Collected with a recursive walk of reversal_of.
--
-- The closed-period check now covers every entry in the chain (not just the
-- statement-line dates), so a reversal posted into a closed period is caught.
-- Reconciliation and posted-only guards are unchanged.
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- Full deletion set: the batch's postings + reclass entries, plus everything
  -- that reverses any of them, transitively.
  WITH RECURSIVE base AS (
    SELECT journal_entry_id AS id FROM public.bank_statement_lines
  ...
  chain AS (
    SELECT id FROM base
    UNION
    SELECT je.id FROM public.journal_entries je JOIN chain c ON je.reversal_of = c.id
  )
  SELECT array_agg(DISTINCT id) INTO v_je_ids FROM chain WHERE id IS NOT NULL;

  -- Closed fiscal period: no entry in the chain may sit in one.
  IF v_je_ids IS NOT NULL THEN
    SELECT count(*) INTO v_n
      FROM public.journal_entries je
      JOIN public.fiscal_periods fp
        ON fp.tenant_id = v_tenant_id AND fp.status = 'closed'
       AND je.entry_date BETWEEN fp.period_start AND fp.period_end
     WHERE je.id = ANY (v_je_ids);
    IF v_n > 0 THEN
      RAISE EXCEPTION 'CLOSED_PERIOD: % entr(ies) fall in a closed fiscal period and cannot be deleted; reopen the period first', v_n
        USING ERRCODE = 'P0001';
    END IF;

    -- Bank-reconciled lines still block a delete.
  ...
  IF v_je_ids IS NOT NULL THEN
    -- Detach reclass links from any surviving lines of OTHER batches that might
    -- point at a chain entry (defensive; normally none).
    UPDATE public.bank_statement_lines SET reclass_journal_entry_id = NULL
     WHERE reclass_journal_entry_id = ANY (v_je_ids);
    UPDATE public.bank_statement_lines SET journal_entry_id = NULL
