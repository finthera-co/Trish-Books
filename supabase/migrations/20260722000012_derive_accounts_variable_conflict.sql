-- ═══════════════════════════════════════════════════════════════════════════
-- Definitive fix for "column reference "derive_key" is ambiguous".
--
-- The RETURNS TABLE OUT columns (derive_key, side, account_id) share names with
-- the bank_import_derived_accounts columns. Alias-qualifying helped the DELETE
-- but the INSERT ... ON CONFLICT (tenant_id, derive_key, side) target list
-- still can't be alias-qualified, so plpgsql (variable_conflict = error) still
-- rejected it. The pragma `#variable_conflict use_column` resolves every such
-- ambiguity to the COLUMN — which is always what we mean in SQL here; the OUT
-- variables are only ever ASSIGNED (unaffected by the pragma), never read
-- inside a SQL statement. Behaviour is otherwise identical.
-- ═══════════════════════════════════════════════════════════════════════════
