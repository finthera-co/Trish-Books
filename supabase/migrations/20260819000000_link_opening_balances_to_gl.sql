-- Link opening_balances (the audit/period-close override table) into the
-- General Ledger, the same way rpc_trial_balance already does.
--
-- Until now, Trial Balance treated a fiscal period's opening_balances row as
-- the authoritative "audit opening" (falling back to the ledger-derived
-- figure when no row exists), but the General Ledger — both the account-tree
-- report (rpc_gl_account_tree / rpc_gl_transactions) and the per-account
-- register (account_opening_balance, behind Ledger.tsx / AccountReport.tsx)
-- — only ever computed opening balances from journal_lines. When a tenant's
-- period-close flow (or manual audit entry) recorded an opening_balances row
-- that differed from what was actually posted, Trial Balance and the General
-- Ledger would show two different opening figures for the same account and
-- period, surfaced only as a dismissible "variance" banner rather than being
-- reconciled. That divergence is now closed: all three functions resolve the
-- enclosing fiscal period for p_date_from and prefer its opening_balances row
-- when one exists, falling back to the ledger-derived amount otherwise —
-- identical logic to rpc_trial_balance's `ob` CTE.
...
AS $$
  ...
  SELECT COALESCE((SELECT d FROM aud), (SELECT d FROM led)),
         COALESCE((SELECT c FROM aud), (SELECT c FROM led));
$$;
...
BEGIN
  ...
  node AS (
    SELECT t.id, t.parent_account_id, t.account_code, t.account_name, t.account_type,
           ...
           COALESCE(ob.aud_open, o.opening, 0) AS own_opening,
...
BEGIN
  ...
  opening AS (
    SELECT COALESCE(led.account_id, ob.account_id) AS account_id,
           COALESCE(ob.aud_open, led.bal, 0)        AS bal
    FROM led
