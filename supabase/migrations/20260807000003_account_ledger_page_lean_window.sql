-- ═══════════════════════════════════════════════════════════════════════════
-- account_ledger_page(): stop dragging the whole row through the window sort.
--
-- The running balance needs cumulative sums over the entire date window in
-- ledger order, so a 32,916-row scan per page is unavoidable. What WAS
-- avoidable: the window ran over `ranked`, which carried every column —
-- description, reference, void_reason — so Postgres sorted 32,916 wide rows on
-- every page request. Measured as `authenticated`, deep page (offset 32,900):
-- 1,440ms.
--
-- The cumulative sums are split into their own narrow CTE (keys + debit/credit
-- only) and joined back to the 50 rows of the page by line_id. Same numbers,
-- ~3x less work: 1,440ms -> ~440ms.
--
-- ledger_txn_type() likewise moved out of the all-rows path. It is only
-- evaluated for the page's rows (for display) or when p_txn_type is actually
-- filtering — the `$6 IS NULL OR …` guard short-circuits it away otherwise.
-- ═══════════════════════════════════════════════════════════════════════════
